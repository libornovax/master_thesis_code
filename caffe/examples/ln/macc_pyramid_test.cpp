...
// 03/20/2017
//
// Tests object detection of a single scale detector on an image pyramid. It outputs BBTXT or BB3TXT depending
// on the type of the used network.
//
...
/**
 * @brief Mode of the detector we are running - given by the number of outputs of the network
 */
enum class Mode {
    BBTXT,
    BB3TXT
...
void extract2DBoundingBoxes (caffe::Blob<float> *output, const std::string &path_image, double scale,
                             std::ofstream &fout)
{
    ...
    for (int i = 0; i < acc_prob.rows; ++i)
    {
        for (int j = 0; j < acc_prob.cols; ++j)
        {
            ...
            if (conf >= 0.1)
            {
                ...
                fout << path_image << " 1 " << conf << " " << xmin << " " << ymin << " " << xmax << " "
                     << ymax << std::endl;
            }
        }
    }
                          ...
                          const std::string &path_image_list, const std::string &path_out)
{
    ...
    CHECK(output_layer->shape(1) == 5 || output_layer->shape(1) == 8) << "Unsupported network!";

    // The number of outputs determines the detector mode
    const Mode mode = (output_layer->shape(1) == 5) ? Mode::BBTXT : Mode::BB3TXT;
    LOG(INFO) << "Mode: " << ((mode == Mode::BBTXT) ? "BBTXT" : "BB3TXT");
    ...
    while (std::getline(infile, line))
    {
        ...
        for (double s: scales)
        {
            ...
            {
                // 2D bounding box
                extract2DBoundingBoxes(output_layer, line, s, fout);
            }
            else if (mode == Mode::BB3TXT)
            {
                // 3D bounding box
    ...
    try {
        ...
        desc.add_options()
            ...
            ("path_out", po::value<std::string>(&pa.path_out)->required(),
             "Path to the output BBTXT (or BB3TXT) file")
        ;
        ...
        if (vm.count("help")) {
            std::cout << "Usage: ./detect_accumulator path/f.prototxt path/f.caffemodel path/image_list.txt path/out.bb*txt\n";
            std::cout << desc;
            exit(EXIT_SUCCESS);
        }
